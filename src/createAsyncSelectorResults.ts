...
export function createAsyncSelectorResults<
  AsyncReturn,
  State,
  Props = undefined,
  DefaultValue = []
...
>(
  ...
  selectors: [
    (state: State, props: Props) => R1,
    (state: State, props: Props) => R2
  ]
): [
  (state: State, props: Props) => AsyncReturn | DefaultValue,
  (state: State, props: Props) => boolean,
...
export function createAsyncSelectorResults(params, selectors: any = []) {
  ...
  const isWaiting = createSelector([asyncSelector], d => d.isWaiting);

  const error = createSelector([asyncSelector], d =>
    d.isRejected ? d.value : null
  );
  ...
  return [results, isWaiting, error as Function, asyncSelector.forceUpdate];
}
